...
lilikoi.machine_learning <- function (MLmatrix = PDSmatrix, measurementLabels = Label,
  ...
  for (k in 1:nrun){
    ...
    if (DL == TRUE){
      ...
      model.list.cart[[k]] = fit.cart
      model.list.lda[[k]]  = fit.lda
      model.list.svm[[k]]  = fit.svm
      model.list.rf[[k]]   = fit.rf
      model.list.log[[k]]  = fit.log
    ...
    model_results$cart <- model.list.cart
    model_results$lda  <- model.list.lda
    model_results$svm  <- model.list.svm
    model_results$rf   <- model.list.rf
    model_results$log  <- model.list.log

    return(c(performance, plots,model_results))
